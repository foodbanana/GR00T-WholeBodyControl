...
> ### 3인칭 사진은 남기지 않았다
> `ego_view` 는 로봇 머리 시점이라 **로봇 자신과 방 전경은 원리적으로 안 찍힌다.**
> 로봇 위치·사람이 붙잡는 자리·바나나까지의 거리 같은 물리 배치는 **문서에 없다.**
>
> 다만 재현에 실제로 필요한 것은 **모델이 보는 화면**이고, 그것이 위 세 장이다.
> 셋업이 위 구도와 같아 보이면 학습 조건과 맞는 것으로 판단한다.
> 조명은 `--color-jitter-params`(밝기 0.3 / 대비 0.4 / 채도 0.5 / 색상 0.08)로 어느 정도
> 흡수되지만 극단적인 변화는 커버되지 않는다.
