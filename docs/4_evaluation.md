...
![양성 에피소드 ep53, ck18000 — 오른팔 7관절](../media/eval_rightarm_ep053_POS_ck18000.png)

*`rightarm_only/ep053_POS_ck18000.png` — 잘 따라간 양성 사례. 빨강(모델)이 파랑(정답 명령)을
전 구간에서 따라간다. 어깨 pitch 가 0° → −145° → 0° 로 크게 움직이는데 RMSE 17.2° 다.*
...
> 위 그림에서 어깨 pitch 의 회색이 파랑보다 **얕게** 내려가는 것이 아래에서 설명할 PD 감쇠다.

파일명에 `POS`/`NEG` 가 박히고 그림 맨 위에 배너로도 표시된다(음성 주황, 양성 초록).

**RMSE 는 가동범위와 같이 볼 것** — `R_wrist_pitch` 는 가동범위가 12~19° 뿐이라 RMSE 0.5° 가
큰 값일 수 있다. y축이 물리적 한계로 고정돼 있어 관절 간 비교가 가능하다. 제목의
`(used N° of M° range)` 가 그 에피소드에서 **실제로 쓴 가동폭**이다.

### 음성 에피소드는 이렇게 생겼다 — 낮은 RMSE 가 실력이 아니다

![음성 에피소드 ep57, ck18000](../media/eval_rightarm_ep057_NEG_ck18000.png)

*`rightarm_only/ep057_NEG_ck18000.png` — 세 곡선이 겹쳐서 한 줄로 보인다.
오른팔이 통틀어 **5.1°** 밖에 안 움직이므로 RMSE 0.2~2.4° 는 당연한 값이다.*

배너가 주황색이고 문구도 `Low RMSE here is expected, not skill.` 이다.
**성능 판단은 양성만으로 한다.** 음성은 "안 움직이는 게 맞다"를 확인하는 용도다.
...
![체크포인트 스윕 — 관절공간 오차 vs val/loss](../media/eval_checkpoint_sweep.png)

**이 문서에서 가장 중요한 그림이다.** 3단 구성이고 측정 하한 **1.53°** 선이 점선으로 같이 그려진다.

| 단 | 무엇을 보는가 |
|---|---|
| **1단** 요약 + val/loss | 빨강(양성 오른팔 RMSE)과 하늘색(val/loss)이 **반대로 간다.** ck4000 이후 val/loss 는 계속 오르는데 관절공간은 평평하다 — 이것이 상관 **−0.605** 의 정체다 |
| **2단** 양성 에피소드별 | 검정 평균선 위로 **ep47(빨강)·ep36(초록) 두 개만 40° 부근에 따로 논다.** 나머지 6개는 12~20° 에 모여 있다 |
| **3단** 음성 에피소드별 | ck2000 이후 전부 측정 하한 점선에 붙는다 = **팔이 정지했다는 뜻** |

세로 점선 두 개가 각각 `val/loss 최적 ck4000`(하늘색)과 `decoded 최적 ck18000`(빨강)이다.
**두 점선이 서로 다른 곳에 있다는 것이 §8-② 의 결론 전부다.**
...
ep36·ep47 은 오차가 아니라 **추종 실패**다.

![추종 실패 사례 — ep47, ck18000](../media/eval_rightarm_ep047_POS_ck18000.png)

*`rightarm_only/ep047_POS_ck18000.png` — 어깨 pitch RMSE **76.0°**. 위의 ep53(17.2°)과
같은 체크포인트, 같은 축이다.*

GT(파랑)는 t=1~4초에 어깨 pitch 를 −145° 까지 내리는데 모델(빨강)은 3.5초까지 0° 부근에
머물다 **+80° 로 반대 방향으로 튄다.** 팔꿈치도 위상이 어긋난 채 진동한다.
반면 손목 3관절은 멀쩡히 따라간다 — **팔 전체가 망가진 것이 아니라 큰 관절만 어긋난다.**

길이·진폭·동작 시점 같은 GT 특성으로는 두 에피소드가 나머지와 구별되지 않는다 — **원인 미상.**
스윕 그림 2단에서 ep47(빨강)·ep36(초록)이 **모든 체크포인트에서 일관되게** 위에 있다는 점이
단서다. 특정 체크포인트의 사고가 아니라 그 두 에피소드에 뭔가가 있다.
...
> ### 단, 대표 그림 4장은 `media/` 에 복사해 커밋했다
> `eval_results/` 규칙을 그대로 두고, **문서가 설명하는 그림만** 골라 옮겼다.
> 클론만 받아도 이 4장은 보인다(`git lfs pull` 필요).
>
> | `media/` 파일 | 원본 | 어디서 쓰나 |
> |---|---|---|
> | `eval_checkpoint_sweep.png` | `checkpoint_sweep.png` | [§6](#6-step-5--체크포인트-스윕-선택의-근거) — 체크포인트 선택 근거 |
> | `eval_rightarm_ep053_POS_ck18000.png` | `rightarm_only/` | [§5](#그래프-읽는-법--세-곡선의-의미가-다르다) — 잘 따라간 양성 |
> | `eval_rightarm_ep057_NEG_ck18000.png` | `rightarm_only/` | [§5](#음성-에피소드는-이렇게-생겼다--낮은-rmse-가-실력이-아니다) — 음성 |
> | `eval_rightarm_ep047_POS_ck18000.png` | `rightarm_only/` | [§8-③](#8-v2-결과--읽고-넘어가야-할-세-가지) — 추종 실패 |
>
> **나머지 80장(에피소드 14 × ck 3 × 2뷰)은 여전히 gitignore 대상**이므로 아래 사슬로 재생성한다.
