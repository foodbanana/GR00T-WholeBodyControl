...
│   g1_deploy_onnx_ref       │         │   keyboard_publisher │
│   --input-type zmq_manager │         │   k / i / p / t      │
│                            │         └───────────┬──────────┘
│   WBC decoder 50Hz         │             :5580   │
│   994-dim obs → 29 action  │                     ▼
└─────────┬──────────────────┘         ⑤ VLA Inference 만 구독한다.
          │                            ③ 은 5580 을 구독하지 않고,
          │                            ⑤ 가 받아 :5556 으로 번역한다
          │ DDS (unitree SDK)
          ▼
...
| ⑤ ↔ ② | ZMQ REQ/REP | **5551 → 5550** | 관측 → **motion_token 40개 chunk** (원격이면 SSH 터널 경유) |
| ⑤ → ③ | ZMQ PUB | 5556 | motion_token 1개씩 (**25Hz**) |
| ③ → ⑤, ⑥ | ZMQ SUB | 5557 | 로봇 상태 (`g1_debug`) |
| ④ → ⑤ | ZMQ PUB | 5580 | 키 입력 `k`/`i`/`p`/`t` (③ 은 구독하지 않는다) |
| ③ → 로봇 | DDS | — | 관절 목표 (50Hz) |
...
**② 블렌딩 프레임 수** — 2026-08-07 실기에서 **`2` 로 확정**됐다.

- `0` — 팔 내릴 때 튄다(경계에서 판단이 뒤집히는 순간)
- `1` — **무효**. `alpha = 1/1 = 1.0` 이라 새 토큰이 그대로 나간다
- **`2`** — ck18000 에서 과제를 유지하며 부드러워졌다 ✅
- `3` — **ck2000 에서 과제를 죽였다**(팔이 안 올라감). 올릴 때는 반드시 양성 조건까지 확인
- 이상하면 `0` 으로 즉시 원복

상세는 [실기 절차 문서](vla_run_procedure.md#2026-08-07-실기-결과) 참조.
