...
[머리 백엔드 2종 — --head-backend]
    ...
    realsense (2026-07-21 실측으로 전제가 바뀜 — 아래 필독):
        ...
        **센서에 직접 요청**하므로 1080p 디코드·리사이즈·재인코딩이 통째로
        ...
        ★ videohub 과의 공존 — **불안정하다(2026-07-21 관측)**
          관측된 사실만 적는다:
            - 머리만 RSUSB(손목 V4L2)로 실행: videohub_pc4 가 **최소 20초 생존**,
              그동안 ego_view 29Hz 정상.
            - 3대 전부 RSUSB + 수 분 실행 후: videohub_pc4 **사라짐**.
          어느 시점에 죽는지, 3대 구성 때문인지 단순 시간 문제인지는 **모른다.**
          ※ 한때 이 자리에 "완전 공존 확정"이라고 적었으나 그것은 pgrep 3번,
            약 20초 관측을 일반화한 것이었다. 취소한다.

          운영상 전제: **videohub 은 언젠가 밀려난다고 가정할 것.**
            - 우리가 명시적으로 정지시키지는 않는다(그럴 필요 없음).
            - 밀려나도 우리 파이프라인은 정상 동작한다(실측 29~30Hz).
            - 복구는 **재부팅**. 부팅 시 videohub 자동 기동 + /dev/video4 =
              D435i 로 원복됨을 2026-07-21 확인했다.
            - ★ videohub 을 수동 기동하지 말 것 — 재열거로 /dev/video4 가
              손목 D405 를 가리키게 될 수 있고, 그러면 우리 손목을 점유한다.
