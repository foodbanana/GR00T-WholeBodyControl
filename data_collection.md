...
- `|Δ| ≥ 33ms`(한 프레임) → 손목이 정상 30Hz가 아니라는 신호. `[sync] WARN` 출력
...
| `STALE_THRESHOLD_MS` | `33.0` | 매칭된 `\|Δ\|`가 이 값 이상이면 stale 경고 |
| `SYNC_REPORT_WINDOW` | `300` | Δ 통계 롤링 윈도(프레임). 30Hz면 ≈10초 |
| `SYNC_REPORT_INTERVAL_S` | `5.0` | `[sync]` 통계 출력 주기(초) |
| `STALE_WARN_INTERVAL_S` | `1.0` | stale 경고 rate-limit(초). 매 발행 스팸 방지 |
